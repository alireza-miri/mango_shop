...
import { addToCart, cartProduct, getOnePost } from "../../redux/action";
import {
  Badge,
  Button,
  Card,
...
const Product = () => {
  ...
  const { loading, data, error } = useSelector((state) => state.OnePost);
  useEffect(() => {
    dispatch(getOnePost(productId));
  }, []);
                ...
                <Col key={item._id}>
                  <Card style={{ width: "30rem", margin: "3rem auto" }}>
                    ...
                    >
                      <Card.Text>
                        <h5>
                          <Badge>price</Badge> ${item.price}
                        </h5>
                      </Card.Text>
                      <Card.Text>
                        <img
                          src="https://img.icons8.com/3d-fluency/24/null/star.png"
                          style={{
                            marginBottom: "6px",
                            color: "black",
                            fontWeight: "bold",
                      ...
                      {item.countInStock ? (
                        <Button
                          ...
                          onClick={() => {
                            {
                              if (
                                !product?.find((prd) => prd._id === item._id)
                              ) {
